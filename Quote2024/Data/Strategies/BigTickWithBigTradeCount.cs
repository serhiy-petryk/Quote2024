...
namespace Data.Strategies
{
    public static class BigTickWithBigTradeCount
    {
        public static void Start()
        {
            ...
            Debug.Print("Ticker\tDateTime\t%\t K trades\tPrevClose\tOpen\tHigh\tLow\tClose\tTrades\tProfit,%");
            using (var zip = ZipFile.Open(zipFileName, ZipArchiveMode.Read))
                                 ...
                                 Path.GetFileNameWithoutExtension(a.Name).Split('_')[1])))
                {
                    ...
                    foreach (var kvp in tickerAndDateAndHighToLow[ticker])
                    {
                        ...
                        for (var k = beforeTickCount; k < data.Length; k++)
                        {
                            var item = data[k];
                            ...
                            Debug.Print($"{ticker}\t{item.DateTime}\t{currentHighToLowPercent}\t{item.n / (1.0 * beforeTrades / beforeTickCount) }\t{item.c}\t{nextTick.o}\t{nextTick.h}\t{nextTick.l}\t{nextTick.c}\t{nextTick.n}\t{(nextTick.c-nextTick.o)/nextTick.o*100}");
                        }
