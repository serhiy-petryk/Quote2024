...
namespace Data.Actions.Investing
{
    public class InvestingSplitsLoader
    {
        ...
        public static void Start()
        {
            ...
            var o = WebClientExt.PostToBytes(URL, postData, true, true, null, null ,true);
        ...
        {
            DbHelper.ClearAndSaveToDbTable(items.Where(a => a.Date <= a.TimeStamp), "dbQ2024..Bfr_SplitInvesting",
                "Symbol", "Date", "Name", "Ratio", "K", "TimeStamp");

            DbHelper.ExecuteSql("INSERT INTO dbQ2024..SplitInvesting (Symbol,[Date],Name,Ratio,K,[TimeStamp]) " +
                               "SELECT a.Symbol, a.[Date], a.Name, a.Ratio, a.K, a.[TimeStamp] FROM dbQ2024..Bfr_SplitInvesting a " +
                               "LEFT JOIN dbQ2024..SplitInvesting b ON a.Symbol = b.Symbol AND a.Date = b.Date " +
